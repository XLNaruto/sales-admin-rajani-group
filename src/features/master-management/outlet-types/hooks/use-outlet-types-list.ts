...
const SORT_BY_COLUMN: Record<string, OutletTypeSortBy> = {
  name: 'type_name',
}

/**
 * Orchestrates the outlet-type master screen: filter/pagination/sort state, the
 * live (server-filtered) list query, the delete flow, and the add/edit modal
 * state. The page consumes this and only renders.
 */
export function useOutletTypesList() {
  const [filters, setFilters] = useState<OutletTypeFilters>(INITIAL_FILTERS)
  ...
  const patchFilters = (patch: Partial<OutletTypeFilters>) => {
  ...
  const infinite = useOutletTypesInfinite(
  ...
  // The list row itself seeds the form (there is no GET-by-id on this master).
  const [modalOpen, setModalOpen] = useState(false)
  const [editRow, setEditRow] = useState<OutletType | null>(null)
  const openCreate = () => {
    setEditRow(null)
    setModalOpen(true)
  }
  const openEdit = (row: OutletType) => {
    setEditRow(row)
    setModalOpen(true)
  }
  ...
  const deleteOutletType = useDeleteOutletType()
  const [pendingDelete, setPendingDelete] = useState<OutletType | null>(null)
  const confirmDelete = () => {
    if (!pendingDelete) return
    const target = pendingDelete
    deleteOutletType.mutate(target.id, {
      onSuccess: () => {
  ...
  return {
    ...
    setPendingDelete,
    confirmDelete,
    isDeleting: deleteOutletType.isPending,
  }
}
