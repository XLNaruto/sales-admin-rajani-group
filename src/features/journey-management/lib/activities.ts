...
/**
 * The solver's reason for a day, rendered to a phrase.
 *
 * The API sends `{ rule, facts }` rather than a sentence, deliberately, so it can
 * be re-rendered in Gujarati later — which means the phrasing lives here and not
 * on the server.
 */
export function solverReasonLabel(
  reason: { rule: string; facts: Record<string, unknown> } | null,
): string {
  if (!reason) return '—'
  const facts = reason.facts ?? {}
  switch (reason.rule) {
    case 'cycle': {
      const visits = Number(facts.visits_per_month)
      if (visits >= 4) return 'Weekly cycle'
      if (visits === 2) return 'Fortnightly cycle'
      if (visits === 1) return 'Monthly cycle'
      return 'Visit cycle'
    }
    case 'half_day_pair':
      return 'Two half-days paired'
    case 'load_balance':
      return 'Load balanced'
    case 'travel_adjacency':
      return 'Nearby beats grouped'
    case 'consecutive_day_avoidance':
      return 'Spread off consecutive days'
    case 'pinned':
      return 'Pinned by you'
    case 'fixed':
      return 'Fixed day'
    default:
      // An unknown rule still tells the reviewer more than an em dash.
      return reason.rule.replace(/_/g, ' ').replace(/^./, (c) => c.toUpperCase())
  }
}
