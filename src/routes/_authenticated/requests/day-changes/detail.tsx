...
import { DayChangeDetailPage } from '@/features/field-requests'
import { decryptParams } from '@/lib/crypto'
import { validateDataSearch } from '@/lib/route-search'
import { requirePermission } from '@/features/permissions'

/**
 * The deep-link target behind a `day_change` notification.
 *
 ...
 * reading "Day Change Request" instead of a bare row number.
 *
 * Gated by the SAME `day-change:list` grant as the queue: reading one request is the same
 ...
 */
export const Route = createFileRoute('/_authenticated/requests/day-changes/detail')({
  beforeLoad: ({ context }) =>
...
function RouteComponent() {
  ...
  return <DayChangeDetailPage id={Number(params.id)} notificationId={params.n} />
}
