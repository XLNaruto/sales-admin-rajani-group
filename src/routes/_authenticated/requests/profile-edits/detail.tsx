...
import { ProfileEditRequestDetailPage } from '@/features/field-requests'
import { decryptParams } from '@/lib/crypto'
import { validateDataSearch } from '@/lib/route-search'
import { requirePermission } from '@/features/permissions'

/**
 * The deep-link target behind a `profile_edit_request` notification.
 *
 ...
 * reading "Profile Edit Request" instead of a bare row number.
 *
 * Gated by the SAME `profile-edit-request:list` grant as the queue: reading one request is the same
 ...
 */
export const Route = createFileRoute('/_authenticated/requests/profile-edits/detail')({
  beforeLoad: ({ context }) =>
...
function RouteComponent() {
  ...
  return <ProfileEditRequestDetailPage id={Number(params.id)} notificationId={params.n} />
}
